...
const SearchMovies: React.FC<Props> = ({ shouldAutoFocus = false }) => {
    ...
    useEffect(() => {
        if (debouncedValue && debouncedValue !== queryState) {
            dispatch(setSearchQuery({query: debouncedValue, totalResults}));
    ...
    return (
        <div className="relative flex max-w-[350px] w-full">
            <input 
                ref={inputRef}
                className="px-5 py-2 grow rounded-xl bg-transparent border border-gray-300 w-full max-w-[350px] font-semibold text-gray-100 placeholder-gray-300 placeholder-opacity-50 focus:outline-none focus:border-amber-500 focus:bg-slate-800 focus:ring-1 focus:ring-amber-600"
                type="text" 
                value={inputValue} 
